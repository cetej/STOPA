...
```
1. Review: read file + TSV + git log + traces
2. Modify: ONE atomic change (if needs "and" → split)
3. Commit: before verification
4. Verify: run metric (spot-check first if 5+ cases)
5. Guard: regression check if guard: configured
6. Decide: improved → keep | same/worse → discard | crash → recover
7. Log to TSV
