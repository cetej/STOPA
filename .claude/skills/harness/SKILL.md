...
| "Schema validates, so the phase output is correct" | Schema-Utility Decoupling (Tool-Genesis arXiv:2603.05578): schema/format compliance ≠ functional correctness (Schema-F1 0.964, SR 0.472). Format passing creates false confidence. | After format validation passes, ALWAYS run a downstream utility check — feed phase output into the NEXT phase's expected input and verify it actually works |
