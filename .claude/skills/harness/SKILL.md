...
| "Schema validates, so the phase output is correct" | Schema/format compliance ≠ functional correctness (Tool-Genesis: Schema-F1 0.964, SR 0.472). Format passing creates false confidence. | After format validation passes, ALWAYS run a downstream utility check — feed phase output into the NEXT phase's expected input and verify it actually works |
