# STOPA — Shared Memory Index

## Orchestration State
- [state.md](state.md) — Active task state shared across agents and skills
- [budget.md](budget.md) — Budget ledger (counters, event log, history)
- [checkpoint.md](checkpoint.md) — Session checkpoint with resume prompt

## Knowledge Base
- [learnings.md](learnings.md) — Patterns, anti-patterns, cross-repo setup, skill gaps
- [decisions.md](decisions.md) — Decision log (what, why, who)
- [news.md](news.md) — /watch scan results (CC updates, AI/ML ecosystem)

## Project Context
- [project_zachvev.md](project_zachvev.md) — Záchvěv: detekce názorových kaskád (EWS, CRI, české NLP)
- [czech_corrector_plan.md](czech_corrector_plan.md) — Czech corrector: Variant A (prompts) → Variant B (hybrid MorphoDiTa+Hunspell)
- [feedback_checkpoints.md](feedback_checkpoints.md) — Checkpoint pravidla: nemixovat, verzovat, častěji ukládat

## Archives
- [implementation-plan.md](implementation-plan.md) — Orchestration v2 plan (all items DONE)
- [budget-archive.md](budget-archive.md) — Archived budget history
- [decisions-archive.md](decisions-archive.md) — Archived old decisions
