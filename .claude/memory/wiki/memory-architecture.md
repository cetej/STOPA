...
Multica (15.4k★ open-source Claude Managed Agents clone) provides external production validation for STOPA's grep-first retrieval philosophy. Its schema uses zero vector embeddings — six relational tables with explicit `JOIN` for skill attachment, JSONB snapshots at dispatch, and append-only audit. The key thesis: "curated relevance beats learned similarity for coding agents — a migration runbook needs the exact runbook, not the statistically-closest one." STOPA's `discovery-keywords:` frontmatter and tag taxonomy are the direct equivalent of Multica's `agent_skill` JOIN. The gaps Multica lacks (fuzzy recall, stale snapshot recovery, cross-workspace memory, skill lifecycle management) are precisely what STOPA's synonym fallback, replay-queue, `/improve` routing, and maturity counters address (ref: 2026-04-19-multica-curated-vs-similarity.md).
...
17. **Curated discovery-keywords over embedding similarity**: for coding agents, exact skill attachment (JOIN) beats similarity search — Multica production evidence with 15.4k★ validates STOPA's tag+keyword approach (ref: 2026-04-19-multica-curated-vs-similarity.md)
...
| [2026-04-19-multica-curated-vs-similarity](../learnings/2026-04-19-multica-curated-vs-similarity.md) | 2026-04-19 | medium | Multica 15.4k★: zero embeddings, relational JOIN for skills — curated relevance beats similarity for coding agents |
