...
Read `~/.claude/memory/pulse.json` (if exists). This is auto-generated by `pulse-generator.py` at SessionStart.
Extract: `system_health.overall`, `system_health.active_projects`, `system_health.signal_coverage`, and per-project summaries from `projects[]` (name, health, active_task).
Also extract `alerts[]` (level, project, message) — show top 3 alerts.
If file missing: skip (show "n/a" in output).

### Step 1b: Read harness audit (if available)
...
projects:       <name1>: <health> | <name2>: <health> | ... (only projects with memory)
pulse_alerts:   <top 3 alerts, one per line> — or "none"
